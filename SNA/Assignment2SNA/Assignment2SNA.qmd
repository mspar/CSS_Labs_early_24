...
> The bigger the island, the more conflict around it and the more 
...
pull_area <- data_graph |> group_by(gainer) |> distinct(area_gained) |> rbind(data.frame(gainer = c("Korea", "Denmark", "India"),
                                                                            area_gained = 0000000000000000.1)) |> ungroup()
...
fastgreedy <- cluster_fast_greedy(graph_undi, modularity = "TRUE")
plot(fastgreedy, graph_undi,
...
plot_dendrogram(fastgreedy, mode="phylo", colbar = palette(), use.edge.length = TRUE)
fastgreedy_tree


#edge.betweenness.community
eb_graph <- cluster_edge_betweenness(graph, directed = T)
plot(eb_graph, graph, vertex.size = 4,
vertex.color = ifelse(V(graph)$independence == TRUE,"Green", "Red"),
edge.color = ifelse(data_graph$conflict == 1 ,'Orange','Blue'),
edge.arrow.size = .2, 
main = 'Gainers and losers of territory with island states involved 1816 - 2015')
...
plot(info_graph, graph, vertex.size = 4,
vertex.color = ifelse(V(graph)$independence == TRUE,"Green", "Red"),
edge.color = ifelse(data_graph$conflict == 1 ,'Orange','Blue'),
edge.arrow.size = .2, 
main = 'Gainers and losers of territory with island states involved 1816 - 2015')
