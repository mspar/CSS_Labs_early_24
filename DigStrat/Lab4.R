...
page_size <- 100
  first_page <- 1
  last_page <- 1101
  sequence <- seq(first_page, last_page, by = 100)
...
urls <- paste0(base_url,1:10)
  ...
  results <- list()
for (i in 1:10) {
  # Bonus: Indicate which page we are currently grabbing by printing
  print(i)
...
for (i in 1:10) {
  ...
  writeLines(page_content, paste0("page",i,"of blog")) # Name the files according to the page
}
...
for (info in news) {
  ...
  href <- xpathSApply(info, './/a[contains(@class, "nyh_teaser__link")]', xmlAttrs)[2]
  title <- xpathSApply(info, './/span[contains(@class, "nyh_teaser__heading-title")]', xmlValue)
  subtitle <- xpathSApply(info, ".//span[contains(@class, 'nyh_teaser__vignette')]", xmlValue)
...
base_url <- "https://www.planecrashinfo.com/database.htm"
urls <- paste0("https://www.planecrashinfo.com/",1920:2024,"/",1920:2024,".htm")
# urls

results <- list()
GET("https://www.planecrashinfo.com/1994/1994.htm")
for (i in 1:104) {
  # Bonus: Indicate which page we are currently grabbing by printing
  print(i)
  results[[i]] <- GET(urls[i])
...
for (i in 1:47) {
  page_content <- content(results[[i]], type = "text", encoding = "ISO-8859-1")
}

page_content <- content(results[[1]], type = "text", encoding = "ISO-8859-1")
