...
matches <- regexpr("Tel:(.*?)(?=[a-zA-Z])", contact, perl = T)
telephone <- regmatches(contact, matches)

pattern <- "Tel:(.*?)(?=[a-zA-Z\\\\])"
match <- stringr::str_extract(contact, pattern)

## THIS ENTIRE PART IS QUESTIONABLY USEFUL; SEEMS BETTER TO JUST EXTRACT EVERYTHING BETWEEN TEL AND WHEN NUMBERS END

# BONUS: Can you make the regex even better?
