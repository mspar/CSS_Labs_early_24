...
response <- GET(url)
print(response) # Check the status code
page_content = content(response, encoding = 'UTF-8', as = 'text')
  writeLines(page_content, "test.htm")
htmlParse(readLines("test.htm", encoding = 'UTF-8'))
...
page_content <- content(response, encoding = 'ISO-8859-1', as = 'text')
...
# 
# passwordE <- "DS24"
# url <- "https://ollion.cnrs.fr/2024/02/14/dss24/"
# 
# parameters <- list(
#   post_password = passwordE
# )
# 
# response <- POST(url, body = parameters, encode = "form")
...
page_content = content(response, encoding = 'ISO-8859-1', as = 'text')
  ...
  htmlParse(readLines("reperdue.htm", encoding = 'ISO-8859-1'))
################################ THE END #######################################