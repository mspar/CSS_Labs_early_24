...
- When doing simply random sampling (plus the chosen alternative), we can estimate the model as if we hadn't sampled.

- Sampling isn't *statistically* free: it comes at the expense of larger standard errors.

- But sampling can make it *computationally* feasible to estimate models that are otherwise infeasible to estimate.
...
us_wrkrs_24 %>% ungroup() |> 
  select(id) %>% 
  mutate(alt=replicate(n(),sample(alts,size=size,replace=F),simplify=F)) %>% 
  unnest(alt) %>% 
...
## Can summarize the occupational outcomes again
...
- Here we implicitly let $\alpha_{ik}$ vary across genders by estimating a separate model for men and women
...
```{r}
# model for women
men<-clogit(choice~p_women+log(n_workers)+strata(id) , 
       data=filter(occ_choice,sex==1))

# model for men
women<-clogit(choice~p_women+log(n_workers)+strata(id) , 
...
```{r}
#| echo: false
modelsummary(list("Men"=men,"Women"=women),
             coef_rename = c("p_women"="Share women",
                             "log(n_workers)"="log(Num. of workers)",
                             "med_age"="Median age (years)",
                             "I(med_age^2)"="Median age squared.",
                             "p_college:I(educ >= 9)FALSE"="No college X share college edu.",
                             "p_college:I(educ >= 9)TRUE"="College deg. X share college edu."),
             title="Coefficients from conditional logit models of occupation choice in the US, 1965-1970",
             notes="Source: Author's calculations using 1970 US Census IPUMS.",
             stars=TRUE,
             gof_map=tibble(raw=c("n","logLik"),
                            clean=c("N person-alts","Log-likelihood"),
                            fmt=c(0,0)))
```
...
Calculating $q_{ij}$ is not always easy. 

Exception: *stratified* sampling of alternatives:

- Divide the set of alternatives into $S$ mutually exclusive strata.
- Index the strata by $s=1,2,...S$
- We indicate the number of alternatives in $s$ by $N_s$
- If we sample $n_s$ alternatives into the choice set from each strata, $s$, then for any alternative $j$ in strata $s$,

$$q_{ij(s)}=n_s/N_s$$

- **NOTE:** when implementing this procedure, we calculate $q$ for the *chosen* alternative *as if* it had been sampled.

## Example of stratification.

- Let's divide the set of 400+ occupations into 4 strata based on the counts of men and women in those occupations:
...
- For women, this leads us to disproportionately sample from women dominated occupations.


## Implementing stratified sampling

First, generate an occupation-level variable indicating the sample strata:
...
us_occs<-us_occs %>% 
  mutate(
    strata=
      1L*(n_workers*(1-p_women)>=lim & n_workers*p_women<lim)+
  ...
  group_by(strata) |> 
  mutate(n_occs_in_strata = n())
```

## Gender composition of the strata

```{r}
us_occs %>% group_by(strata) %>% 
  summarize(`N occupations`=n(),
            `Share female`=mean(p_women))
```
...
us_wrkrs_24 %>% 
  select(id,occ) %>% 
  expand_grid(select(us_occs,alt=occ5yr,strata))  |> 
  filter(alt!=occ) |> 
  group_by(id,strata) |> 
  slice_sample(n=size) |> 
  ungroup() |> 
  mutate(choice=F) |> 
  select(id,alt,choice) ->
  occ_choice_q
```
...
us_wrkrs_24 |>  
  ...
  left_join(us_occs, by=join_by(alt==occ5yr)) |> 
  arrange(id,strata,desc(choice)) |> 
  group_by(id,strata) |>
  slice(1:size) |> 
  group_by(id,strata) |>
  mutate(q=n()/n_occs_in_strata) |> 
  left_join(us_wrkrs_24,by=join_by(id)) -> 
  occ_choice_q
```
...
```{r}
sum(occ_choice_q$choice)==nrow(us_wrkrs_24)
```
```{r}
occ_choice_q |> group_by(strata) |> summarize(n())
```

## Estimate the model

```{r, size="small"}
clogit(choice~p_women+log(n_workers)+
         offset(-log(q))+strata(id) , 
       data=filter(occ_choice_q,sex==2))
```

## Alternate estimation package

```{r, size="small"}
mclogit::mclogit( cbind(choice,id) ~ 
                    p_women + log(n_workers) + 
                    offset(-log(q)) , 
       data=filter(occ_choice_q,sex==2))
```
...
- We introduce variance in our estimates when we sample (too many) alternatives that are unlikely to be chosen.

- It may be better to choose alternatives that we believe, *a priori*, our respondents will be most likely to choose.
...
## Two approaches to testing differences in coefficients
...
c1<-clogit(choice ~ log(n_workers) + 
             p_women + strata(id), 
                      data = d)

c2<-clogit(choice ~ log(n_workers) + 
             p_women:sex + strata(id), 
                      data = d)
```
...
```{r, echo=F}
modelsummary::modelsummary(list(c1,c2),
