...
# here, sex needs to be made into factor
# country variable needs to be broken down


data_mult |> select(sex, age, citizen, rachsing, degfield, degfield_collapsed, empstat, uhrswork, inctot, incearn, poverty, young_migrant, bpl_collapsed) |> 
  filter(empstat == "Employed" & poverty != 0) -> data_mult

data_mult <- data_mult |> group_by(young_migrant) |> slice_sample(n = 10000) |> ungroup()

library(nnet)
library(broom)
library(modelr)

multi <- multinom(degfield_collapsed ~ sex + age + young_migrant + bpl_collapsed, data = data_mult, trace = F, model = T, na.action = na.exclude)
tidy(multi) |> View()

library(modelsummary)

modelsummary(multi, shape =  term ~ model + response,
             fmt = 2,
             statistic = 'conf.int',
             title = "title",
             exponentiate = TRUE,
