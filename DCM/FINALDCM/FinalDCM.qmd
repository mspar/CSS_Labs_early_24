...
this_works <- clogit(choice ~ mean_weekly_hours + alt + strata(ID), data = degree_choice2)
tidy(this_works) |> View()

this_doesnt_work <- clogit(choice ~ as.numeric(uhrswork) + alt + strata(ID), data = degree_choice2)
...
# filtered stuff
ym <- clogit(choice ~ alt + log(n_degree_holders) + strata(ID) ,
       data = filter(degree_choice2, young_migrant == "YM"))
tidy(ym) |> View()

nym <- clogit(choice ~ alt + log(n_degree_holders) + strata(ID) ,
